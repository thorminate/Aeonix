import path from "path";
import url from "url";
import Interaction, { InteractionTypes } from "#core/interaction.js";
import InteractionManager from "#manager/interactionManager.js";

type Holds = Interaction<
  InteractionTypes.MentionableSelectMenu,
  boolean,
  boolean,
  boolean,
  boolean
>;

const __dirname = path.dirname(url.fileURLToPath(import.meta.url));

export default class MentionableSelectMenuManager extends InteractionManager<Holds> {
  getKey(instance: Holds): string {
    const key = instance.data.data.custom_id;
    if (!key) throw new Error("No custom_id found in mentionableSelectMenu");
    return key;
  }

  folder(): string {
    return path.join(__dirname, "..", "content", "mentionableSelectMenus");
  }
}
